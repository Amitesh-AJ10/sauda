...
export function DealList({ deals, selectedId, onSelect }: DealListProps) {
  if (deals.length === 0) {
    return (
      <p data-testid="deal-list-empty" className="rounded-[20px] border border-black bg-white p-4 text-sm text-black/60">
        No leads yet. Waiting for the first WhatsApp message…
      </p>
    )
  }
  ...
  return (
    <div data-testid="deal-list" className="flex flex-col gap-3">
      {deals.map((deal) => (
        <DealCard key={deal.id} deal={deal} selected={deal.id === selectedId} onSelect={() => onSelect(deal.id)} />
      ))}
    </div>
  )
}
